...
- **New section: server-rendered copy for the App Router.** A pure `makeCatalogT(catalog, locale)` built on the SDK's exported `interpolate` — translated text in the served bytes, ICU intact, 80/80 correct with 8 locales × 10 rounds concurrent, concurrency-safe by construction. With an explicit warning not to seed the global stores from a Server Component, and the diagnostic that catches it: compare each response against its own expected content, not against the other responses (a contamination-shaped test reports zero bleed on a page that is wrong every time).
- **Pages Router: documented that seeding during render removes the flash.** Seeding synchronously in `_app` from `pageProps` showed the correct locale at 42ms with a stale catalog in `localStorage` and no hydration mismatch, where the `init()`-in-`useEffect` form does not. Paired with the rule that makes it safe — the seed and every read must sit in one uninterrupted render pass — and the note that the same placement bleeds during SSR when something suspends between them.

- **`translationsLoadingPromise` documented as settling on failure.** The README said it fires "after the new translations arrive" and showed a `.then()` callback for re-rendering. It resolves on a failed fetch too: `translationsLoadingPromise` is the SDK's internal `change()` promise, and on failure the fetch helper *returns normally rather than throwing* (CJS `dist/index.js:682`, returning before the `sTranslations.set()` at `:695`; the ESM `dist/index.mjs` is a constant 2 lines lower), so `change()` resolves exactly as it does on success while the catalog still holds the previous locale. It never rejects. Following the old guidance runs your "translations arrived" callback against stale data and renders fluent wrong-language content.
- **The two signals are not coherent with each other.** The locale write sits inside a `setTimeout(…, 100)` (CJS `dist/index.js:696`), so on a *successful* fetch there is a ~100ms window where the promise has settled and `useCurrentLocale()` still reports the old locale. `settled && current !== requested` is therefore not a failure test — written as one it produces a flapping error state on every successful switch. The docs now say to treat a match as the only positive, and state plainly that **there is no reliable failure signal**: a failed fetch and one that succeeded 50ms ago are indistinguishable from outside, so an error state needs your own timeout.
- **Checking the locale instead is not the fix — it hangs.** `currentlyLoadedLocale` is written only on the success path (CJS `dist/index.js:696`), so after a failed fetch it never updates and a component gating purely on `useCurrentLocale() === requested` waits forever. The docs now recommend using both edges, which is exactly what the SDK exposes and no more: the promise is the only "it ended" signal, the locale the only "it worked" signal. The locale write is also deferred 100ms behind the promise, so it must be observed from a later render rather than read inside `.then()`.
- A locale whose fetch fails leaves the last successful catalog in `localStorage`, persisted with no locale tag, until a reload, `LangsysApp.refresh()`, or a switch to a *different* locale triggers another fetch. Re-selecting the same failed locale does not retry.
- Applied to `README.md`, the `<LocaleSwitcher>` example in `README-SSR.md`, and the `translationsLoadingPromise` getter's JSDoc — that one had none, and JSDoc ships in `dist/` as IDE hover text.
