...
- **Server messages.** `resolveServerMessages(body, { key | resolver, pieces? })` and `renderServerMessage(entry, category?)` re-exported by reference, with `DEFAULT_SERVER_MESSAGE_CATEGORY` and the `ServerMessage` / `ResolveServerMessagesOptions` / `ServerMessagePieces` types; `messagesCategory` init option inherited from the base config. Entries are read only from where the server attached them — a configured `key` or an app `resolver` — beside the framework's own error body, which is never searched or changed; `field` and `code` are the framework's own, passed through. **`useRenderServerMessage()`** renders entries in components and re-renders them when the locale changes: the translation of the entry's `template` when the catalog has one, its `message` otherwise. Works unchanged for the entries an Inertia server hands to the page after a failed form.
- **Legacy-key migration.** The core's `legacyKeys` init option (inherited from the base config) lets an app keep calling `t()` / `useT()` with its i18next, vue-i18n or plain JSON keys: a key the files hold registers and renders as its converted value, never as the key. `LegacyFormatError` and the `LegacyKeyFile` type are re-exported; `LangsysApp.Translations.setLegacyKeys` turns the mode on or off later.
- **Catalog snapshots.** `LangsysApp.loadSnapshot(snapshot, locale?)` (the core's, by reference) loads an exported catalog synchronously before the first render; the fetched catalog still replaces it. `SnapshotError` and the `CatalogSnapshot` type are re-exported.
