...
- **Server messages.** `resolveServerMessages(body, { key?, resolver? })` and `renderServerMessage(entry, category?)` re-exported by reference, with `SERVER_MESSAGE_CODES`, `DEFAULT_SERVER_MESSAGE_CATEGORY` and the `ServerMessage` / `ResolveServerMessagesOptions` types; `messagesCategory` init option inherited from the base config. **`useRenderServerMessage()`** renders entries in components and re-renders them when the locale changes: the translation of the entry's `template` when the catalog has one, its `message` otherwise. Works unchanged for the entries an Inertia server hands to the page after a failed form.
