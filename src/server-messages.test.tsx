...
/**
 * `useRenderServerMessage` against the shared server-message vectors (spec MSG-5).
 *
 ...
 * fallback decision is the core's. What this proves is the binding's half: every render row
 * comes out of a React component through the hook, and the component re-renders when the
 * catalog arrives after it has mounted.
 */
...
const doc = JSON.parse(raw.toString('utf8')) as { render: RenderRow[]; canonical_entries: ServerMessage[] };
...
describe('useRenderServerMessage', () => {
    it('re-renders a mounted list when the catalog arrives', () => {
        const [, , password] = doc.canonical_entries;
        publish(null, 'es');
        ...
        act(() =>
            publish(
                { Errors: { [password.template]: 'La contraseña debe tener al menos {min} caracteres.' } },
                'es',
            ),
        );
        expect(el.textContent).toBe('La contraseña debe tener al menos 12 caracteres.');
    });
    ...
    it('changes identity when the catalog changes, so a memoised child re-renders', () => {
        ...
        expect(host.textContent).toBe('La contraseña debe tener al menos 12 caracteres.');
    });

    it('renders the entries resolved from a langsys envelope', () => {
        const body = { status: false, error: { message: 'x', code: 'validation_failed', template: 'x', errors: doc.canonical_entries.slice(2) } };
        publish({ Errors: { 'The password confirmation does not match.': 'La confirmación de la contraseña no coincide.' } }, 'es');
        const el = mount(resolveServerMessages(body).filter((e) => e.field));
        expect([...el.querySelectorAll('li')].map((li) => li.textContent)).toEqual([
            doc.canonical_entries[2].message,
