...
"
    while IFS= read -r commit; do
        if [ -n "$commit" ]; then
            ...
            COMMIT_MESSAGES="$COMMIT_MESSAGES- [\`$COMMIT_HASH\`](https://github.com/langsys/langsys-js-react/commit/$COMMIT_HASH)
  $FULL_COMMIT_MSG
...
\`\`\`bash
npm install langsys-js-react@$NEW_VERSION
\`\`\`$COMMIT_MESSAGES

## Full Changelog

See the [commit history](https://github.com/langsys/langsys-js-react/compare/v$ORIGINAL_VERSION...v$NEW_VERSION) for a complete diff of all changes."
...
log_info "Watch the run: https://github.com/langsys/langsys-js-react/actions"
log_success "Release page: https://github.com/langsys/langsys-js-react/releases/tag/v$NEW_VERSION"
log_success "npm (once CI finishes): https://www.npmjs.com/package/langsys-js-react/v/$NEW_VERSION"
